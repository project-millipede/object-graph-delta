...
/**
 * Merges the provided options with the library's default configuration.
 *
 * Default settings:
 * - `trackCircularReferences`: `true` (prevents stack overflows).
 * - `arrayPolicy`: `'diff'` (traverses arrays by index).
 * - `arrayEquality`: `'shallow'` when `arrayPolicy: 'atomic'` and omitted.
 * - `keysToSkip`: `[]` (skips nothing).
 *
 * @param options - The user-provided options.
 * @returns A complete `Options` object with all fields initialized.
 */
export function normalizeOptions(options: DiffOptions = {}): Options {
  const trackCircularReferences = options.trackCircularReferences ?? true;
  const keysToSkip = options.keysToSkip ?? [];

  if (options.arrayPolicy === 'atomic') {
    return {
      trackCircularReferences,
      arrayPolicy: 'atomic',
      arrayEquality: options.arrayEquality ?? 'shallow',
      keysToSkip
    };
  }

  if (options.arrayPolicy === 'ignore') {
    return {
      trackCircularReferences,
      arrayPolicy: 'ignore',
      keysToSkip
