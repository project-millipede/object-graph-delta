...
type DiffArrayOptions = {
  /**
   * Controls the array policy used during diffing.
   *
   * - "diff":
  ...
  /**
   * Not applicable for `arrayPolicy: 'diff'`.
   */
  arrayEquality?: never;
};

type IgnoreArrayOptions = {
  /**
   * Controls the array policy used during diffing.
   *
   * - "ignore":
  ...
  /**
   * Not applicable for `arrayPolicy: 'ignore'`.
   */
  arrayEquality?: never;
};

type AtomicArrayOptions = {
  /**
   * Controls the array policy used during diffing.
   *
   * - "atomic":
  ...
  /**
   * Defines the comparison logic used to detect changes when `arrayPolicy` is set to `'atomic'`.
...
/**
 * Internal normalized options used by the diff engine.
 * `arrayEquality` is only present in `arrayPolicy: 'atomic'` mode.
 */
export type Options = SharedOptions &
  (DiffArrayOptions | IgnoreArrayOptions | AtomicArrayOptions);

/**
 * Public options accepted by `diff()`.
 *
 * Notes:
 * - `arrayPolicy` defaults to `'diff'` when omitted.
 * - `arrayEquality` is only allowed when `arrayPolicy: 'atomic'`.
 */
export type DiffOptions = Partial<SharedOptions> &
  (
    | {
        arrayPolicy?: 'diff';
        arrayEquality?: never;
