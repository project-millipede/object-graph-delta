 ...
 *
 * @param options - The full options object containing `arrayPolicy` and `arrayEquality` settings.
...
export function getArrayStrategy<V>(options: Options): ArrayStrategy<V> {
  switch (options.arrayPolicy) {
    ...
    default:
      throw new Error('Invalid array policy.');
  }
}
 ...
 * 1. Atomic/Ignore:
 *    Handles the comparison immediately and returns any resulting diffs.
 *    Signals `shouldRecurse: false` because the array is treated as a leaf or ignored.
 * 2. Diff:
 *    Returns `shouldRecurse: true`, signaling the caller to iterate over the
 *    array indices and compare them individually.
 *
