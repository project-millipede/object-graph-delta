import type { Container, DiffOptions, DiffResult, Node } from '../types';
 ...
 *
 * @template V - The leaf value type contained in the compared structures.
 * @returns A diff runner configured with `arrayPolicy: 'diff'`.
 */
export function createArrayDiffRunner<V = unknown>(): DiffRunner<V> {
  return (input: DiffInput<V>) =>
    diff(input.previous, input.current, {
      trackCircularReferences: input.options?.trackCircularReferences,
      keysToSkip: input.options?.keysToSkip,
      arrayPolicy: 'diff'
    });
}
 ...
 *
 * @template V - The leaf value type contained in the compared structures.
 * @returns A diff runner configured with `arrayPolicy: 'atomic'` and `arrayEquality: 'reference'`.
 */
 ...
 *
 * @template V - The leaf value type contained in the compared structures.
 * @returns A diff runner configured with `arrayPolicy: 'atomic'` and `arrayEquality: 'shallow'`.
 */
 ...
 *
 * @template V - The leaf value type contained in the compared structures.
 * @returns A diff runner configured with `arrayPolicy: 'ignore'`.
 */
